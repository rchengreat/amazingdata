...
with DAG(
    ...
    schedule=["20 15 * * 1-4", "20 16 * * 5"],
    start_date=datetime(2026, 4, 28),
    catchup=False,
    max_active_runs=1,
    tags=["amazingdata", "stock", "info", "daily"],
    description="工作日 15:20（周五 16:20）拉取 info_stock_basic 和 info_stock_factor",
) as dag:
    ...
    fetch_stock_info = BashOperator(
        ...
        bash_command=(
            _NAS_STATS + "; "
            + DOCKER_CMD.format(script="fetch_stock_info.py")
            + "; "
            + _NAS_STATS
            + "; exit 0"
        ),
