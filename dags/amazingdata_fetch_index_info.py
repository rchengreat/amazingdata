...
_DOCKER_CMD = (
    ...
    "python3 scripts/fetch_index_info.py"
)
...
def run_fetch_index_info(**context):
    ...
    results = {}
    for fname in ["info_index_detail_history.parquet", "info_index_weight_history.parquet"]:
    ...
    if failed:
        raise RuntimeError(f"fetch_index_info 部分失败: {failed}")
...
def send_summary_email(**context):
    ...
    results = ti.xcom_pull(task_ids="fetch_index_info", key="result") or {}
    ...
    subject = (
        f"✅ amazingdata_fetch_index_info 执行成功 - {date_str}"
        if overall_ok
        else f"⚠️ amazingdata_fetch_index_info 部分失败 - {date_str}"
    )

    body = (
        f"amazingdata_fetch_index_info 执行报告\n\n"
        f"执行时间: {time_str}\n\n"
        f"文件结果:\n"
        + "\n".join(rows)
...
with DAG(
    ...
    description="工作日 15:55拉取 info_index_detail_history 和 info_index_weight_history",
) as dag:

    fetch_index_info = PythonOperator(
        task_id="fetch_index_info",
        python_callable=run_fetch_index_info,
    ...
    fetch_index_info >> summary_email
